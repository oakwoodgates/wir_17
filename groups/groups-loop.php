...
if ( bp_is_user() ) {
	$user_favs = WIR_Favorites::get_user_favorite_groups( bp_displayed_user_id() );
	$exclude = implode(',', $user_favs);
	$string = bp_ajax_querystring( 'groups' ) . '&exclude='. $exclude;
//	$string = bp_ajax_querystring( 'groups' );
} else {
	$string = bp_ajax_querystring( 'groups' ); 
}

if ( bp_has_groups( $string ) ) :
